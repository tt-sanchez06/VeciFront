...
function detectApiBase(){
  try {
    if (typeof window !== 'undefined'){
      if (window.API_BASE) return normalizeApiBase(window.API_BASE);
      const meta = typeof document !== 'undefined' ? document.querySelector('meta[name="api-base"]') : null;
      if (meta && meta.content) return normalizeApiBase(meta.content.trim());
...
async function api(path, { method='GET', data, formData }={}){
  ...
  const bases = (() => {
    const list = [];
    const explicitBase = normalizeApiBase((typeof window !== 'undefined' && window.API_BASE) || API_BASE);
    if (explicitBase) {
      list.push(explicitBase);
    } else {
      list.push('');
    }
    try {
      ...
      if (!explicitBase) {
        ...
        if (port && port !== '3000') list.push('http://localhost:3000', 'http://127.0.0.1:3000');
      }
    } catch {
      if (!explicitBase) list.push('http://localhost:3000');
    }
...
function createSocketConnection(options = {}){
  ...
  const base = normalizeApiBase(window.API_BASE || API_BASE);
  const url = base || undefined;
  return window.io(url, { transports: ['websocket'], withCredentials: true, ...options });
}
