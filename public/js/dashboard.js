...
async function sendActiveChatMessage(){
  ...
  socket.emit('send_message', { solicitudId: selectedConversation.solicitudId, toUserId, mensaje: text });
  chatEls.input.value = '';
}
...
function handleTyping(){
  ...
  socket.emit('typing', { solicitudId: selectedConversation.solicitudId, toUserId });
}
...
socket.on('auth_ok', ()=>{
  ...
  joinedSolicitudes.forEach(id => socket.emit('join_solicitud', id));
});
...
socket.on('new_message', (m)=>{
  ...
  if (selectedConversation && Number(selectedConversation.solicitudId) === sid){
    ...
    if (m.id_receptor === (user?.id)){
      socket.emit('mark_read', { messageId: m.id, solicitudId: sid });
    }
    conv.unreadCount = 0;
  } else if (m.id_receptor === (user?.id)){
...
socket.on('typing', ({ solicitudId: sid, fromUserId })=>{
  ...
  if (chatEls.typing){
    ...
    chatTypingTimeout = setTimeout(()=>{ chatEls.typing.textContent = ''; }, 1200);
  }
});

socket.on('read', (info)=>{
  if (!selectedConversation || Number(selectedConversation.solicitudId) !== Number(info.solicitudId)) return;
  const nodes = Array.from(chatEls.box?.children || []);
